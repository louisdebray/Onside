...
// Certains flux (Gazzetta, AS...) renvoient jusqu'à 100 items d'un coup ; les traduire
// tous épuiserait vite le quota gratuit de traduction. On ne traduit que les plus
// récents, largement suffisant pour ce qui est affiché.
...
export async function getGeneralNews() {
  ...
  for (const feed of NEWS_FEEDS) {
    try {
      const items = (await fetchFeed(feed.url)).slice(0, MAX_ITEMS_PER_FEED);
