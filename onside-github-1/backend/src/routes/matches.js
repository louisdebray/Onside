...
import { getMatches } from '../sources/footballData.js';
...
router.get('/:competitionCode', async (req, res) => {
  ...
  const key = `matches:${competitionCode}:${season || 'current'}`;
  ...
  try {
    ...
    console.error('matches: échec appel football-data', err.message);
